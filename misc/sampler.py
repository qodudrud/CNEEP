...
class CartesianSampler:
    ...
    def __init__(self, M, L, batch_size, device="cpu", train=True, autoencoder=False):
        self.size = M * (L - 1)
        self.M = M
        self.L = L
        ...
        if self.training:
            if self.ae:
                ...
                traj_idx = torch.randint(
                    0, self.L, (self.batch_size,), device=self.device
                )
...
class CartesianSeqSampler(CartesianSampler):
    def __init__(self, M, L, n, batch_size, device="cpu", train=True):
        ...
        Examples::
            ...
            >>>     print(batch)
        """
        super(CartesianSeqSampler, self).__init__(M, L, batch_size, device=device, train=train)
        self.size = M * (L - n + 1)
